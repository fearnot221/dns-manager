...
function ManageDialog({ unitId, target, onClose, onSaved }: { unitId: string; target: Member | "rotate"; onClose: () => void; onSaved: (code?: string) => Promise<void> }) {
  return <ActionDialog title={target === "rotate" ? "重設單位 passcode？" : `管理成員 · ${target.label}`} description={target === "rotate" ? "舊加入碼會立即失效，不影響既有成員。新碼只顯示一次。" : "至少需保留一位可登入的單位管理員。移除成員會同時作廢舊加入碼，之後請重設並發送新碼。"} onClose={onClose} label="確認更新" submit={async (form) => { const role = form.get("role"); const result = await apiRequest<{ passcode?: string }>(`/api/units/${encodeURIComponent(unitId)}`, jsonRequest("PATCH", target === "rotate" ? { action: "rotate" } : { action: "member", userId: target.userId, role: role === "REMOVE" ? null : role })); await onSaved(result.passcode); }}>{target !== "rotate" && <label>成員角色<Select name="role" aria-label="成員角色" defaultValue={target.role} options={[...Object.entries(unitRoleLabels).map(([value, label]) => ({ value, label })), { value: "REMOVE", label: "移出單位" }]} /></label>}</ActionDialog>;
}
