...
export function ZonesTable() {
  ...
  async function saveAccess(zone: ZoneRow, enabled: boolean) {
    if (pending) return;
    setPending(zone.id); setFailure(""); setNotice("");
    try {
      ...
      setClosing(null);
    } catch (error) { setFailure(error instanceof Error ? error.message : "儲存失敗，請重試。"); }
  ...
  return <>
    ...
    <div className="card table-card">
      ...
      <div className="table-tools">
        ...
        <button type="button" className="button" disabled={loading || !!pending} onClick={() => { setOverrides({}); setFailure(""); setNotice(""); void load(); }}><RefreshCw size={14} />重新整理</button>
      </div>
      ...
      {!loading && !error && rows.length === 0 && <EmptyState title={query || accessFilter !== "all" ? "此分類沒有符合條件的網域" : `目前沒有可管理的${selected.label}`} description="請調整搜尋、申請狀態，或切換其他分類。" />}
      </div>
    </div>
    {closing && <Dialog title="暫停此網域的申請？" description={`${closing.name} 將從申請表移除，既有 DNS 與待審核申請不受影響。`} pending={!!pending} onClose={() => { setClosing(null); setFailure(""); }}>
      ...
      <div className="modal-actions"><button className="button" type="button" disabled={!!pending} onClick={() => setClosing(null)}>取消</button><button className="button primary" type="button" disabled={!!pending} onClick={() => void saveAccess(closing, false)}>{pending ? "儲存中…" : "暫停申請"}</button></div>
    </Dialog>}
