...
export function ZonesTable() {
  ...
  const rows = useMemo(() => filterZones(zones, category, query), [zones, category, query]);
  ...
  return <>
    <div className="card table-card">
      ...
      <div className="table-tools">
        ...
        <button type="button" className="button" disabled={loading} onClick={refresh}><RefreshCw size={14} className={loading ? "spin" : ""} aria-hidden="true" />重新整理</button>
      </div>
      ...
      {!loading && !error && rows.length === 0 && <EmptyState title={query ? "此分類沒有符合條件的網域" : `目前沒有可管理的${selected.label}`} description="請調整搜尋，或切換其他分類。" action={query && <button type="button" className="button" onClick={clearQuery}>清除此分類的篩選</button>} />}
      </div>
    </div>
  </>;
}
