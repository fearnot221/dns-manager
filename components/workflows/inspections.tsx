...
function InspectionResponseForm({ pending, onSubmit }: { pending: boolean; onSubmit: (form: HTMLFormElement) => void }) {
  ...
  return <form className="form-surface message-reply" onSubmit={(event) => { event.preventDefault(); onSubmit(event.currentTarget); }}>
    <fieldset className="form-fields inspection-response" disabled={pending}>
      <label>確認結果<Select name="status" aria-label="確認結果" value={result} onChange={setResult} options={[{ value: "CONFIRMED", label: "確認仍在使用" }, { value: "ISSUE", label: "回報問題" }]} /></label>
...
export function InspectionsWorkbench({ admin, actorId }: { admin: boolean; actorId: string }) {
  ...
  return <div className="workflow-page">
    <div className="admin-toolbar"><label>通知狀態<Select aria-label="通知狀態" value={status} onChange={setStatus} options={[{ value: "ALL", label: "全部" }, ...Object.entries(labels).map(([value, label]) => ({ value, label }))]} /></label><button className="button" disabled={loading || pending} onClick={() => void reload()}>重新整理</button></div>
