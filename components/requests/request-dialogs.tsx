...
export function ReviewDialog({ review, onClose, onSaved }: { review: { item: DnsRequest; decision: "APPROVE" | "REJECT" }; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success(review.decision === "APPROVE" ? "已核准，DNS 紀錄已生效" : "已標記為未核准");
      await onSaved();
  ...
  return <Dialog title={approving ? "核准這筆 DNS 申請？" : "不核准這筆 DNS 申請？"} description={approving ? "核准後會立即套用 DNS，請確認申請類別與新舊解析內容。" : "申請人會看到你的審核回覆。"} pending={pending} onClose={onClose}><form onSubmit={submit}><fieldset className="dialog-fields" disabled={pending}>
    <div className="modal-body"><div className="review-record"><Badge tone={review.item.recordType === "A" ? "orange" : "neutral"}>{review.item.recordType}</Badge><strong>{review.item.recordName}</strong>{review.item.sourceRecordId && <p>原內容：<code>{review.item.originalContent}</code></p>}<p>{review.item.sourceRecordId ? "新內容：" : "解析內容："}<code>{review.item.content}</code></p><span>{review.item.zoneName} · {review.item.ttl}s TTL</span></div><label>審核回覆 <small>{approving ? "選填" : "建議填寫原因"}</small><textarea name="reviewNote" rows={3} maxLength={1000} placeholder="說明審核結果或需要補充的資料" /></label>{approving && <div className="warning"><AlertTriangle size={17} /><div><strong>即時變更 DNS</strong><p>{review.item.sourceRecordId ? "只替換申請指定的原解析值，其餘紀錄及 TTL 維持不變。" : "同名稱、同類型的既有紀錄會保留。"}</p></div></div>}{error && <div ref={errorRef} tabIndex={-1} className="form-error" role="alert"><AlertTriangle size={15} aria-hidden="true" />{error}</div>}</div>
    <div className="modal-foot"><button className="button" type="button" disabled={pending} onClick={onClose}>取消</button><SubmitButton pending={pending} label={approving ? "核准並套用紀錄" : "確認不核准"} /></div>
