...
export function DnsRequestsWorkbench({ admin, actorId }: { admin: boolean; actorId: string }) {
  ...
  return <>
    ...
    <div className="request-toolbar">
      <div className="filter-input request-search"><Search size={15} aria-hidden="true" /><input ref={searchInput} type="search" value={query} onKeyDown={(event) => { if (event.key === "Escape" && query) { event.preventDefault(); clearSearch(); } }} onChange={(event) => setQuery(event.target.value)} placeholder={admin ? "搜尋名稱、Identifier、姓名、單位或用途" : "搜尋名稱、網域、內容或用途"} aria-label="搜尋 DNS 申請" />{query && <button type="button" className="search-clear" onClick={clearSearch} aria-label="清除搜尋"><X size={15} /></button>}</div>
