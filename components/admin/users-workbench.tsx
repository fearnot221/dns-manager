...
function UserDialog({ user, canAssignAdmin, onClose, onSaved }: { user: UserRow; canAssignAdmin: boolean; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Dialog title={"管理帳號"} description={user.protected ? "最高權限帳號僅可修改備註，角色與狀態仍受保護。" : "以停用代替刪除，保留申請與清查歷史。"} onClose={onClose} pending={pending}><form onSubmit={submit}><fieldset className="dialog-fields" disabled={pending}><div className="modal-body"><label>帳號<input value={user.account} readOnly /></label><label>帳號備註<textarea name="note" defaultValue={user.note} maxLength={1000} rows={3} placeholder="僅供管理用途，不影響帳號權限" /></label>{canAssignAdmin && !user.protected && <label>角色<select name="globalRole" defaultValue={user.globalRole}><option value="USER">一般使用者</option><option value="ADMIN">管理員</option></select></label>}{!user.protected && <label>帳號狀態<select name="disabled" defaultValue={String(user.disabled)}><option value="false">使用中</option><option value="true">停用</option></select></label>}{error && <div className="form-error" role="alert">{error}</div>}</div><div className="modal-foot"><button className="button" type="button" onClick={onClose} disabled={pending}>取消</button><SubmitButton pending={pending} label={"儲存變更"} /></div></fieldset></form></Dialog>;
}
