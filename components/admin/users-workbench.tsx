...
function UserDialog({ user, canAssignAdmin, onClose, onSaved }: { user: UserRow; canAssignAdmin: boolean; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Dialog title={"管理帳號"} description="管理帳號資訊與備註；角色與狀態依權限開放調整。" onClose={onClose} pending={pending}><form onSubmit={submit}><fieldset className="dialog-fields" disabled={pending}><div className="modal-body"><label>電子郵件<input value={user.email || "未提供電子郵件"} readOnly /></label><label>顯示名稱<input value={user.name} readOnly /></label><label>帳號備註<textarea name="note" value={note} onChange={(event) => setNote(event.target.value)} aria-describedby={noteHelp} data-dialog-initial-focus maxLength={1000} rows={3} placeholder="僅供管理用途，不影響帳號權限" /><small id={noteHelp} className="user-note-help"><span>管理用途，不影響權限</span><span>{note.length} / 1000 字</span></small></label>{(canAssignAdmin || user.protected) && <label>角色<select name="globalRole" disabled={user.protected} defaultValue={user.protected ? "ADMIN" : user.globalRole}><option value="USER">一般使用者</option><option value="ADMIN">管理員</option></select></label>}{<label>帳號狀態<select name="disabled" disabled={user.protected} defaultValue={String(user.disabled)}><option value="false">使用中</option><option value="true">停用</option></select></label>}{error && <div ref={errorRef} tabIndex={-1} className="form-error" role="alert">{error}</div>}</div><div className="modal-foot"><button className="button" type="button" onClick={onClose} disabled={pending}>取消</button><SubmitButton pending={pending} label={"儲存變更"} />{canAssignAdmin && !user.protected && <button className="button danger" type="button" disabled={pending} onClick={async () => { if (!window.confirm("移除此帳號？將撤銷登入及單位權限、輪替其單位加入碼，保留歷史。單位最後一位管理員須先移交。")) return; setPending(true); setError(""); try { await apiRequest(`/api/users/${user.id}`, jsonRequest("DELETE", { confirmId: user.id })); await onSaved(); } catch (e) { setError(e instanceof Error ? e.message : "移除失敗"); } finally { setPending(false); } }}>移除帳號</button>}</div></fieldset></form></Dialog>;
}
