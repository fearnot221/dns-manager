...
type UserRow = { id: string; name: string; email: string | null; account: string; note: string; globalRole: "USER" | "ADMIN" | "SUPER_ADMIN"; disabled: boolean; protected: boolean; zoneAdmin: boolean };
export function UsersWorkbench() {
  ...
  const rows = (data?.users || []).filter((u) => `${u.email || ""} ${u.name} ${u.account} ${u.note}`.toLowerCase().includes(query.trim().toLowerCase()));
  return <><div className="admin-toolbar users-toolbar"><div className="filter-input"><Search size={16} aria-hidden="true" /><input ref={searchInput} type="search" placeholder="搜尋電子郵件、帳號或備註" aria-label="搜尋使用者" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(event) => { if (event.key === "Escape" && query) { event.preventDefault(); clearSearch(); } }} />{query && <button type="button" className="search-clear" aria-label="清除使用者搜尋" onClick={clearSearch}><X size={15} /></button>}</div><button type="button" className="button" disabled={loading} onClick={() => void reload()}><RefreshCw size={15} aria-hidden="true" className={loading ? "spin" : ""} />重新整理</button><span className="users-toolbar-hint muted">帳號於 Portal 首次登入後建立</span></div>
    {!loading && !error && <p className="record-results" role="status">顯示 {rows.length} / {data?.users.length ?? 0} 個帳號</p>}
    {error ? <ResourceError message={error} retry={reload} /> : loading ? <p role="status" className="record-results">正在載入使用者…</p> : <div className="card table-card"><ScrollRegion className="table-wrap" label="使用者清單，可水平捲動"><table><thead><tr><th scope="col">電子郵件／顯示名稱</th><th scope="col">角色</th><th scope="col">狀態</th><th scope="col">操作</th></tr></thead><tbody>{rows.map((user) => <tr key={user.id}><td className="user-account-cell"><strong>{user.email || "未提供電子郵件"}</strong><p className="user-note muted">{user.name} · 帳號：{user.account}</p>{user.note.length > 100 ? <details className="user-note-detail"><summary aria-label={`展開 ${user.account} 的完整備註`}>{user.note.slice(0, 100)}…</summary><p>{user.note}</p></details> : <p className="user-note muted">{user.note || "尚無備註"}</p>}</td><td>{user.protected ? "管理員" : user.globalRole === "ADMIN" || user.zoneAdmin ? "管理員" : "一般使用者"}</td><td><Badge tone={user.disabled ? "neutral" : "green"}>{user.disabled ? "已停用" : "使用中"}</Badge></td><td>{user.protected ? <><Badge>受保護</Badge> <button className="button" onClick={() => setEditing(user)}>編輯備註</button></> : <button className="button" disabled={!canEdit(user)} title={!canEdit(user) ? "需要管理員指派權限" : "管理使用者"} onClick={() => setEditing(user)}>管理</button>}</td></tr>)}</tbody></table></ScrollRegion>{!rows.length && <div className="empty"><p>{query.trim() ? "沒有符合搜尋條件的帳號。" : "目前沒有使用者帳號。"}</p>{query && <button type="button" className="button" onClick={clearSearch}>清除搜尋</button>}</div>}</div>}
...
function UserDialog({ user, canAssignAdmin, onClose, onSaved }: { user: UserRow; canAssignAdmin: boolean; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Dialog title={"管理帳號"} description={user.protected ? "受保護帳號僅可修改備註，角色與狀態仍受保護。" : "以停用代替刪除，保留申請與清查歷史。"} onClose={onClose} pending={pending}><form onSubmit={submit}><fieldset className="dialog-fields" disabled={pending}><div className="modal-body"><label>電子郵件<input value={user.email || "未提供電子郵件"} readOnly /></label><label>顯示名稱<input value={user.name} readOnly /></label><label>帳號備註<textarea name="note" value={note} onChange={(event) => setNote(event.target.value)} aria-describedby={noteHelp} data-dialog-initial-focus maxLength={1000} rows={3} placeholder="僅供管理用途，不影響帳號權限" /><small id={noteHelp} className="user-note-help"><span>管理用途，不影響權限</span><span>{note.length} / 1000 字</span></small></label>{canAssignAdmin && !user.protected && <label>角色<select name="globalRole" defaultValue={user.globalRole}><option value="USER">一般使用者</option><option value="ADMIN">管理員</option></select></label>}{!user.protected && <label>帳號狀態<select name="disabled" defaultValue={String(user.disabled)}><option value="false">使用中</option><option value="true">停用</option></select></label>}{error && <div ref={errorRef} tabIndex={-1} className="form-error" role="alert">{error}</div>}</div><div className="modal-foot"><button className="button" type="button" onClick={onClose} disabled={pending}>取消</button><SubmitButton pending={pending} label={"儲存變更"} />{canAssignAdmin && !user.protected && <button className="button danger" type="button" disabled={pending} onClick={async () => { if (!window.confirm("移除此帳號？將撤銷登入及單位權限、輪替其單位加入碼，保留歷史。單位最後一位管理員須先移交。")) return; setPending(true); setError(""); try { await apiRequest(`/api/users/${user.id}`, jsonRequest("DELETE", { confirmId: user.id })); await onSaved(); } catch (e) { setError(e instanceof Error ? e.message : "移除失敗"); } finally { setPending(false); } }}>移除帳號</button>}</div></fieldset></form></Dialog>;
}
