...
type UserRow = { id: string; email: string | null; account: string; note: string; globalRole: "USER" | "ADMIN" | "SUPER_ADMIN"; disabled: boolean; protected: boolean; zoneAdmin: boolean };
export function UsersWorkbench() {
  ...
  const rows = (data?.users || []).filter((u) => `${u.email || ""} ${u.account} ${u.note}`.toLowerCase().includes(query.trim().toLowerCase()));
