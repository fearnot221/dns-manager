...
export function UsersWorkbench() {
  ...
  return <><div className="admin-toolbar"><div className="filter-input"><input placeholder="搜尋姓名或電子郵件" aria-label="搜尋使用者" value={query} onChange={(e) => setQuery(e.target.value)} /></div><button className="button primary" disabled={loading || !!error} onClick={() => setEditing("new")}>新增使用者</button></div><div className="request-notice">fearnot@ce.ncu.edu.tw 為受保護的最高使用者，無法在後台修改。只有此帳號可新增或指派管理員。</div>
