...
export function AuditWorkbench() {
  ...
  return <>
    <form className="admin-toolbar audit-toolbar" onSubmit={(event) => { event.preventDefault(); setSearch(query.trim()); setPage(1); }}>
      <div className="filter-input"><input ref={searchInput} type="search" maxLength={200} aria-label="搜尋操作紀錄" placeholder="姓名、電子郵件、網域或請求編號" value={query} onChange={(event) => setQuery(event.target.value)} /></div>
