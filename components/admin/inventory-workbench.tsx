...
export function InventoryWorkbench({ systemAdmin = false }: { systemAdmin?: boolean }) {
  ...
  return <><div className="admin-toolbar card"><div className="filter-input"><Search size={16} aria-hidden="true" /><input ref={searchInput} type="search" onKeyDown={(event) => { if (event.key === "Escape" && query) { event.preventDefault(); clearSearch(); } }} aria-label="搜尋 DNS 清查" placeholder="搜尋名稱、IP、申請人、單位或用途" value={query} onChange={(e) => setQuery(e.target.value)} />{query && <button type="button" className="search-clear" aria-label="清除清查搜尋" onClick={clearSearch}><X size={15} /></button>}</div><label>檢視<select value={view} onChange={(e) => setView(e.target.value as typeof view)}><option value="inspection">清查列表</option><option value="table">表格</option><option value="list">列表</option><option value="grid">IP 棋盤</option><option value="type">依 Type</option><option value="name">依 Name</option><option value="content">依 Content</option></select></label><label>分組<select value={group} onChange={(e) => setGroup(e.target.value as InventoryGroup)}><option value="applicantUnit">依單位</option><option value="applicantName">依申請人</option><option value="purpose">依用途</option><option value="zoneName">依網域</option></select></label><label>顯示<select value={status} onChange={(e) => setStatus(e.target.value as InventoryFilter)}><option value="all">全部紀錄</option><option value="unreviewed">尚未清查</option><option value="reviewed">已有清查紀錄</option><option value="missing">歸屬資料未完整</option></select></label><button className="button" disabled={loading} onClick={() => void reload()}><RefreshCw size={15} className={loading ? "spin" : ""} />重新整理</button></div>
