...
import { filterRequests, statuses, type DnsRequest } from "@/components/requests/model";

const item: DnsRequest = { id: "one", zoneName: "example.com.", recordName: "Lab.example.com.", recordType: "TXT", content: "test", ttl: 300, status: "PENDING", createdAt: "2026-09-14T00:00:00Z", canReview: false, user: { id: "u", email: "", studentId: "115500001", accounts: [{ providerAccountId: "portal-user" }] }, applicantUnit: "實驗室", applicantExtension: "1234", purpose: "研究服務", reviewNote: "請補說明" };
it("searches visible identity, contact and purpose fields ignoring whitespace and case", () => {
  for (const query of [" LAB ", "115500001", "portal-user", "實驗室", "1234", "研究服務", "請補說明"]) expect(filterRequests([item], query, "ALL")).toEqual([item]);
});
