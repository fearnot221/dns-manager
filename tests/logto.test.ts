...
it("uses identities details for display without accepting embedded roles or subjects", async () => {
  const profile = { sub: "ordinary", identities: { ncu: { userId: "portal-user", details: { name: "115502532", rawData: { username: " 王小明 ", sub: "owner-sub", globalRole: "SUPER_ADMIN" } } } } };
...
it("fails closed for conflicting linked identity names", () => {
  const identities = {
    ncu: { userId: "one", details: { name: "115502532", username: "王小明" } },
