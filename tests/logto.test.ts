...
it("prefers Chinese-name claims, keeps student IDs auxiliary and never uses them for roles", () => {
  expect(logtoIdentity({ sub: "opaque", "chinese-name": " 王小明 ", chineseName: "其他姓名", name: "English Name", "student-id": "115502532" })).toMatchObject({ name: "王小明", hasName: true, studentId: "115502532" });
  expect(logtoIdentity({ sub: "opaque", chineseName: "王小明" }).name).toBe("王小明");
  expect(logtoIdentity({ sub: "opaque", "chinese-name": " ", name: "映射姓名" }).name).toBe("映射姓名");
