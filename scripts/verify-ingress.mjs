...
try {
  ...
  const mock = "const http=require('node:http'); for(const port of [3000,9000]) http.createServer((req,res)=>{res.setHeader('Content-Type','application/json');res.end(JSON.stringify({port,path:req.url,host:req.headers.host,proto:req.headers['x-forwarded-proto']}))}).listen(port,'127.0.0.1');";
  ...
  const probe = "(async()=>{for(let i=0;i<30;i++){try{const a=await fetch('http://127.0.0.1:8080/login');const b=await fetch('http://127.0.0.1:8080/hooks/github',{method:'POST'});if(!a.ok||!b.ok)throw Error();const web=await a.json(),hook=await b.json();if(web.port!==3000||web.host!=='dns.ce.ncu.edu.tw'||web.proto!=='https'||hook.port!==9000||hook.path!=='/hooks/github')process.exit(2);console.log('Private ingress routing, HTTPS headers and webhook path verified');return}catch{await new Promise(r=>setTimeout(r,200))}}process.exit(1)})()";
