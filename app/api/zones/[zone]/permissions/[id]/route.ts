...
async function DELETEHandler(request:Request,{params}:{params:Promise<{zone:string;id:string}>}){
  ...
  try{
    ...
    else{const permission=await db.zonePermission.findFirst({where:{id:resolved.id,zoneName:zone},include:{user:{select:{globalRole:true,accounts:{where:{provider:{in:["ncu-portal","logto"]}},select:{provider:true,providerAccountId:true}}}}}});if(permission&&accountOwnerIdentifier(permission.user.accounts, permission.user.globalRole)===OWNER_IDENTIFIER)throw new ApiError("最高使用者受保護。",403);previous=await db.zonePermission.findFirst({where:{id:resolved.id,zoneName:zone}});if(!previous)throw new ApiError("Permission not found",404);await db.zonePermission.delete({where:{id:resolved.id}});}
