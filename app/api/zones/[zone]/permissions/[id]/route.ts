...
async function DELETEHandler(request:Request,{params}:{params:Promise<{zone:string;id:string}>}){
  ...
  try{
    ...
    if(!process.env.DATABASE_URL){const index=mockPermissions.findIndex((item)=>item.id===resolved.id);if(index<0)throw new ApiError("Permission not found",404);if(isOwnerEmail(mockPermissions[index].user.email)||mockPermissions[index].userId==="dev-owner")throw new ApiError("最高使用者受保護。",403);previous=mockPermissions[index];mockPermissions.splice(index,1);}
    else{const permission=await db.zonePermission.findFirst({where:{id:resolved.id,zoneName:zone},include:{user:{select:{email:true}}}});if(permission&&isOwnerEmail(permission.user.email))throw new ApiError("最高使用者受保護。",403);previous=await db.zonePermission.findFirst({where:{id:resolved.id,zoneName:zone}});if(!previous)throw new ApiError("Permission not found",404);await db.zonePermission.delete({where:{id:resolved.id}});}
