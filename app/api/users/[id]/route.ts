...
async function PATCHHandler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const check = (user: { email: string; name?: string | null; disabled?: boolean; globalRole: "USER" | "ADMIN" | "SUPER_ADMIN"; zoneAdmin?: boolean }) => { if (!mayManageUser(actor, user)) throw new ApiError("此帳號受保護，無法修改。", 403); before = { email: user.email, name: user.name, disabled: user.disabled, globalRole: user.globalRole }; };
    ...
    await logAuditEvent({ actor, zone: "", action: "UPDATE_USER", before, after: { id, email: user.email, name: user.name, disabled: user.disabled, globalRole: user.globalRole }, success: true, request });
