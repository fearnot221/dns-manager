...
async function PATCHHandler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const check = (user: { email: string; name?: string | null; portalIdentifier?: string | null; note?: string; disabled?: boolean; globalRole: "USER" | "ADMIN" | "SUPER_ADMIN"; zoneAdmin?: boolean }) => { if (!mayManageUser(actor, user)) throw new ApiError("此帳號受保護，無法修改。", 403); before = { account: user.portalIdentifier || "未綁定 Portal", note: user.note || "", disabled: user.disabled, globalRole: user.globalRole }; };
    const user = isLocalDemo() ? await demoUsers((users) => { const user = users.find((u) => u.id === id); if (!user) throw new ApiError("找不到使用者。", 404); check({ ...user, portalIdentifier: user.id === "dev-owner" ? "115502532" : user.portalIdentifier }); Object.assign(user, input); return user; }, true) : await db.$transaction(async (tx) => {
