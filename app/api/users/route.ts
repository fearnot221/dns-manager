...
export async function GET() {
  try {
    ...
    const users = isLocalDemo() ? await demoUsers((items) => items.map((user) => ({ ...user, portalIdentifier: user.id === "dev-owner" ? "115502532" : user.portalIdentifier, zoneAdmin: false }))) : (await db.user.findMany({ select: { id: true, studentId: true, portalEmail: true, note: true, accounts: { where: { provider: "ncu-portal" }, select: { providerAccountId: true } }, email: true, globalRole: true, disabled: true, createdAt: true, zonePermissions: { where: { role: "ADMIN" }, select: { id: true } }, groupMemberships: { select: { group: { select: { zonePermissions: { where: { role: "ADMIN" }, select: { id: true } } } } } } } })).map(({ zonePermissions, groupMemberships, ...user }) => ({ ...user, zoneAdmin: !!zonePermissions.length || groupMemberships.some((item) => item.group.zonePermissions.length > 0) }));
    return Response.json({ users: users.map(accountPresentation), canAssignAdmin: isOwner(actor) });
