...
export async function PUT(request: Request) {
  try {
    ...
    await logAuditEvent({ actor, zone: input.zoneName, recordName: input.recordName, recordType: input.recordType, action: input.mode === "inspect" ? "INSPECT_DNS_RECORD" : "UPDATE_DNS_OWNERSHIP", after: { recordId: input.id, mode: input.mode }, success: true, request });
