...
- `AUTH_LOGTO_ID`、`AUTH_LOGTO_SECRET`。最高學號來自已驗證的 UserInfo `identities[*].details.name`，不是 env。
...
既有帳號由維運人工核對 Logto User ID 後綁定，不自動依 Email 合併；最高帳號登入時須在一致的 NCU identity 回傳 `details.name=115502532`。正式 SSO 流程需另行驗收。
