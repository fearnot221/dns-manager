...
export type AuditEvent = { id: string; createdAt: string; userId: string | null; userEmail: string; zone: string; recordName?: string | null; recordType?: string | null; action: string; oldValue?: unknown; newValue?: unknown; ipAddress?: string | null; userAgent?: string | null; requestId: string; success: boolean; errorMessage?: string | null };
...
export async function logAuditEvent(input:AuditInput):Promise<void>{
  ...
  else if (isLocalDemo()) await localDocument<{ events: AuditEvent[] }, void>("audit", async () => ({ events: [] }), (store) => { store.events.push({ ...data, id: crypto.randomUUID(), createdAt: new Date().toISOString() }); }, true);
