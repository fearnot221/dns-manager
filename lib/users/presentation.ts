...
export function accountPresentation(user: { id: string; name?: string | null; email: string; portalEmail?: string | null; globalRole: GlobalRole; studentId?: string | null; note?: string; portalIdentifier?: string | null; disabled?: boolean; zoneAdmin?: boolean; accounts?: { provider?: string; providerAccountId: string }[] }) {
  ...
  return { id: user.id, name: personDisplay({ name: user.name, email, studentId: user.studentId }).primary, email, studentId: user.studentId, account: portalIdentifier || email || `未綁定 Portal (${user.id})`, note: user.note || "", portalIdentifier, globalRole: resolvedGlobalRole(user.email, user.globalRole, portalIdentifier), disabled: !!user.disabled, zoneAdmin: !!user.zoneAdmin, protected: portalIdentifier === OWNER_IDENTIFIER };
}
