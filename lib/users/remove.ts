...
import { isOwner, accountOwnerIdentifier, OWNER_IDENTIFIER } from "@/lib/auth/owner";
...
export async function removeUser(actor: Actor, id: string) {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (id === actor.id || accountOwnerIdentifier(user.accounts, user.globalRole, user.logtoName) === OWNER_IDENTIFIER) throw new ApiError("此帳號不能移除。", 403);
