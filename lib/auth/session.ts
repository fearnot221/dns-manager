...
export async function requireActor():Promise<Actor>{
  ...
  return {id:user.id,email:user.email,portalEmail:user.portalEmail,name:user.name,portalIdentifier:accountOwnerIdentifier(user.accounts, user.globalRole),studentId:user.studentId,globalRole:resolvedGlobalRole(user.email,user.globalRole,accountOwnerIdentifier(user.accounts, user.globalRole)),zoneRoles};
}
