...
const profileSchema = z.object({ sub: z.string().min(1).max(200), "chinese-name": displayName, chineseName: displayName, name: displayName, "student-id": studentId, studentId, email: z.email().optional().nullable(), email_verified: z.boolean().optional() });
export function logtoIdentity(raw: unknown) {
  const profile = profileSchema.parse(raw);
  const name = profile["chinese-name"] || profile.chineseName || profile.name;
  return { id: profile.sub, hasName: !!name, name: name || "未提供姓名", studentId: profile["student-id"] || profile.studentId || null, portalEmail: profile.email_verified === true ? profile.email?.toLowerCase() || null : null,
...
export function logtoProvider(): OIDCConfig<Record<string, unknown>> {
  return {
    ...
    profile(raw) { const identity = logtoIdentity(raw); return { id: identity.id, email: identity.email, name: identity.name, studentId: identity.studentId, portalEmail: identity.portalEmail, globalRole: "USER" }; },
    account() { return {}; },
